...
export type OmAiUsageEventName =
  | 'om-ai.call.started'
  | 'om-ai.call.ended'
...
export interface OmAiAccountProfile {
  id: EntityId;
  email: string;
  displayName: string;
  avatarUrl?: string;
...
export interface OmAiAccountPreferences {
  language: string;
...
export interface OmAiBillingSubscription {
  id: EntityId;
  appId: OmAiAppId;
  planId: OmAiPlanId;
...
export interface OmAiBillingUsage {
  appId: OmAiAppId;
  quota: {
    callMinutesDaily: number | null;
  };
  used: {
    callMinutesToday: number;
  };
  remaining: {
    callMinutesToday: number | null;
  };
}

export type OmAiBetaGateReason =
  | 'eligible'
  | 'subscription_inactive'
  | 'plan_not_eligible';

export interface OmAiBetaGate {
  isUnlocked: boolean;
  reason: OmAiBetaGateReason;
  status: OmAiBillingSubscription['status'];
  planId: OmAiPlanId;
  summary: string;
}

export type OmAiProviderId = 'openai-realtime' | 'openai-responses' | 'fallback-mock';

export type OmAiProviderHealth = 'healthy' | 'degraded' | 'down';

export type OmAiProviderRegistrySource = 'memory-store';

export type OmAiProviderCapabilityId =
  | 'live-call'
  | 'recap-generation'
  | 'persona-response';

export interface OmAiProviderRegistryItem {
  id: OmAiProviderId;
  name: string;
  capabilities: OmAiProviderCapabilityId[];
  priority: number;
  health: OmAiProviderHealth;
}

export interface OmAiProviderRuntimeMetrics {
  providerId: OmAiProviderId;
  p95LatencyMs: number;
  errorRate: number;
}

export interface OmAiProviderRouteDecision {
  appId: OmAiAppId;
  capability: OmAiProviderCapabilityId;
  providerId: OmAiProviderId | null;
  providerName: string | null;
  reason: string;
  fallbackProviderId: OmAiProviderId | null;
  score: number;
}

export interface OmAiProviderRegistryResponse {
  appId: OmAiAppId;
...
export interface OmAiProviderObservabilityItem extends OmAiProviderRegistryItem {
  source: OmAiProviderRegistrySource;
...
export interface OmAiProviderObservabilitySummary {
  totalProviders: number;
  healthyProviders: number;
  degradedProviders: number;
  downProviders: number;
  averageP95LatencyMs: number | null;
  averageErrorRate: number | null;
}

export interface OmAiProviderObservabilityResponse {
  appId: OmAiAppId;
  source: OmAiProviderRegistrySource;
  lastSyncedAt: ISODateTimeString;
  providers: OmAiProviderObservabilityItem[];
  routeDecisions: OmAiProviderRouteDecision[];
  summary: OmAiProviderObservabilitySummary;
}
