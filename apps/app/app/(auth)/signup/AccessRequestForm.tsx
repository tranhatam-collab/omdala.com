...
export function AccessRequestForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setStatus({
      tone: "info",
      message: isVi
        ? "Đang gửi yêu cầu truy cập..."
        : "Sending your access request...",
    });

    try {
      await apiJsonRequest(
        "/v1/auth/access-request",
        {
          method: "POST",
          headers: {
      ...
      <form className="auth-form" onSubmit={handleSubmit}>
        ...
        <label>
          ...
          <input
            ...
            placeholder={
              isVi
                ? "Tên node hoặc tổ chức của bạn"
                : "Your node or organization name"
            }
            required
          />
        </label>
        <label>
          ...
          <textarea
            ...
            placeholder={
              isVi
                ? "Bạn đang xây dựng, điều phối hoặc khám phá điều gì?"
                : "What you are building, coordinating, or exploring."
            }
          />
        </label>
        <button
