...
import { clearClientSession, persistClientSession } from "@/lib/session-client";
    ...
    void (async () => {
      try {
        const data = await apiJsonRequest<{
          ...
          `/v1/auth/magic-link?token=${encodeURIComponent(token)}&next=${encodeURIComponent(nextPath)}`,
          { method: "GET" },
          isVi
            ? "Không thể xác thực magic link."
            : "Unable to verify your magic link.",
        );
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await apiJsonRequest(
        ...
        isVi ? "Không gửi được magic link." : "Unable to send your magic link.",
      );
  ...
  return (
    <>
      ...
      <p className="auth-note">
        ...
        <a href={`mailto:${OMDALA_INBOXES.noreply}`}>
          {OMDALA_INBOXES.noreply}
        </a>
        . {isVi ? "Nếu cần hỗ trợ, phản hồi qua " : "For support, reply via "}
        <a href={`mailto:${OMDALA_INBOXES.support}`}>
          {OMDALA_INBOXES.support}
        </a>
        .
      </p>
