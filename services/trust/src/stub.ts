...
function buildBlockers(verificationStatus: VerificationStatus, proofCount: number): string[] {
  ...
  if (verificationStatus !== 'verified') {
    blockers.push('Hoàn tất xác minh để mở mức hiển thị mạnh hơn.')
  }

  if (proofCount < 3) {
    blockers.push('Bổ sung thêm bằng chứng từ công việc đã hoàn tất hoặc hoạt động thực tế.')
  }
  ...
  return {
    ...
    highlights: [
      input.verificationStatus === 'verified'
        ? 'Lớp xác minh đã hoàn tất.'
        : 'Xác minh vẫn là đòn bẩy tăng trưởng quan trọng.',
      input.proofCount > 0
        ? `Đã có ${input.proofCount} tín hiệu được hậu thuẫn bằng bằng chứng.`
        : 'Chưa có tín hiệu nào được gắn bằng chứng.',
    ],
