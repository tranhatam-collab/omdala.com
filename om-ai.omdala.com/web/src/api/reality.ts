...
export async function getPlan(planId: string): Promise<ApiCallResult<PlanRecord>> {
  const result = await getJson<ApiEnvelope<{ plan: PlanRecord }>>(`/v2/reality/plans/${planId}`);
  if (result.error || !result.data?.data.plan) return fail(result.error ?? 'request_failed');
  return { value: result.data.data.plan, error: null };
}

export async function createApproval(runId: string): Promise<ApiCallResult<ApprovalRecord>> {
...
export async function getApproval(approvalId: string): Promise<ApiCallResult<ApprovalRecord>> {
  const result = await getJson<ApiEnvelope<{ approval: ApprovalRecord }>>(`/v2/reality/approvals/${approvalId}`);
  if (result.error || !result.data?.data.approval) return fail(result.error ?? 'request_failed');
  return { value: result.data.data.approval, error: null };
}
