...
## 0. Boundary Lock Update — April 9, 2026

This file contains valuable historical architecture context, but its original framing is now broader than the current product split.

Current canonical rule:

- `Om AI` = AI human call / learning / communication / companion / business interaction product
- `Om AI` is **not** `OmCode`
- `Om AI` is **not** `Omniverse`
- physical environment control, rooms, devices, scenes, and automation belong to `Omniverse`

If this document conflicts with root canonical planning docs, follow:
...
5. `AI_OM_FOUNDER_READY_FINAL_HANDOFF_PACK_2026.md`
6. `AI_OM_SHARED_PLATFORM_DEPENDENCY_STATUS_2026.md`
7. `AI_OM_TEAM_BOUNDARY_AND_DEPENDENCY_MATRIX_2026.md`
8. `AI_OM_SHARED_RESOURCE_REUSE_PLAN_2026.md`
9. `AI_OM_PHASED_INTEGRATION_BACKLOG_2026.md`
10. `AI_OM_CONTROL_PASS_STATUS_2026.md`

This means Team Om AI planning docs now explicitly control:

- dependency status with shared platform
- team boundary and ownership rules
- resource reuse strategy
- phased integration sequencing
- founder-ready final handoff interpretation

Current execution priority:

- `Om AI Live` is the active build path
- any retained `Reality` / `device` / `gateway` sections below are preserved as historical split context or bridge-only references unless root canonical docs re-promote them

## 1. Executive Summary

Om AI is now positioned as the AI human interaction product inside OMDALA.

Its primary surface is `Om AI Live`: the user calls an AI human in real time for learning, speaking, coaching, guided reflection, business interaction, and wellness support.

Historical reality/device material in this file should now be read as legacy-transition context, not as the current primary product scope.

The correct category today is therefore:

**Om AI = AI Human Interaction OS**

This means one app focuses on live voice or video interaction with specialized AI humans such as:
...
The product must be built as native-first, call-grade, policy-first, proof-first, and memory-first. iOS remains the strategic anchor because Apple frameworks expose the highest-value call surfaces natively. The backend remains the system of record for memory, metering, moderation, subscriptions, and provider orchestration.
...
Om AI = AI Human Call Platform + Learning and Communication Layer

### 2.3 Core Positioning

Your AI Human Interaction OS
