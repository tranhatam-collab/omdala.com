...
Status: Production verified with auth migration in progress (2026-04-06)
...
- Dedicated auth host flow is enabled (`app -> auth -> app`) with cookie-scoped sessions.
...
- Auth Pages project is bound to `auth.omdala.com`.
...
- Prod smoke passes for:
  ...
  - dashboard auth gate behavior under missing session conditions (redirect to auth host)
...
```bash
pnpm --filter @omdala/auth build
pnpm --filter @omdala/auth typecheck
```

Expected:

- Auth surface builds and serves login flow on `auth.omdala.com`.
- Magic-link token exchange endpoint (`/v1/auth/session/exchange`) sets secure cookies for `.omdala.com`.
...
## 7) Ops / Reliability Verification
...
## 12) One-Command Deploy Pipeline
...
Explicit Pages production branch mapping:

- `app` -> `main`
- `auth` -> `production`
- `web` -> `main`

Backwards-compatible legacy aliases still exist:

```bash
pnpm release:deploy:api-only
pnpm release:deploy:app-only
pnpm release:deploy:auth-only
...
pnpm release:deploy:auth-only:verify
